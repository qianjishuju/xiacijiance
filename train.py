...
from nets.model.Labs.labs import Labs
from utils.check import check_amp
from nets.training_utils import (get_lr_scheduler, set_optimizer_lr,
                                              weights_init)
from utils.callbacks import LossHistory, EvalCallback
from data.AutoLoaderSelector import auto_ds_sel, deeplab_dataset_collate
...
def main():
    ...
    DATASET_PATH = config["base"].get("dataset_path", '圆识别数据集')
    ...
    if CUSTOM_DS:
        ...
        if DS_File and Path(DS_File).is_file() and Path(DS_File).suffix == ".py":
            ...
            print(
                "[red bold]:warning: :warning: :warning: You are importing a custom DatasetsLoader, which may contain malicious code")
    ...
    # ----------------------------------------------------------------------------------------------------------------------------#
    model_path = "/home/lx/.lab_model/hgnetv2l.pt"
    ...
    if ARCH.lower() == "unet":
        ...
        from nets.model.hardnet import hardnet
        model = hardnet(num_classes=num_classes, pretrained=pretrained)
    elif ARCH.lower() == "unetpp":
        from nets.model.UNetplusplus.UnetPP import UNetPlusPlus
    ...
    #     from 贴胶识别训练代码.utils.get_featuremap import get_featureMap
    ...
    if DS_File and Path(DS_File).is_file() and Path(DS_File).suffix == ".py":
        ...
        train_dataset = auto_ds_sel(input_shape, num_classes, True, VOCdevkit_path, jitter_prop=jitter, flip_prop=flip,
                                    blur_prop=blur)
    ...
    if True:
        ...
        # ----------------------#
        if local_rank == 0:
            eval_callback = EvalCallback(model, input_shape, num_classes, val_dataset.annotation_lines, VOCdevkit_path,
                                         log_dir, cuda, \
